...
function thsp_cbp_customize_register( $wp_customize ) {
	...
	foreach( $thsp_sections as $thsp_section_key => $thsp_section_value ) {
		...
		foreach( $thsp_section_fields as $thsp_field_key => $thsp_field_value ) {
			...
			$thsp_field_value['setting_args']['sanitize_callback'] = 'thsp_cbp_sanitize_cb';
			...
			if ( ! isset( $thsp_field_value['setting_args']['capability'] ) ) {
				$thsp_field_value['setting_args']['capability'] = 'manage_options';
			}

			if ( ! isset( $thsp_field_value['setting_args']['transport'] ) ) {
				$thsp_field_value['setting_args']['transport'] = 'postMessage';
			}
		    ...
		    [transport] => refresh
		    [sanitize_cb] => thsp_cbp_sanitize_cb
		    [sanitize_callback] => thsp_cbp_sanitize_cb
			*/

			/*
						echo '<pre>';
						print_r($thsp_field_value['setting_args']);
						echo '</pre>';
			*/
			...
			$wp_customize->add_setting(
				$setting_control_id,
				array(
				      "default" => (array_key_exists("default",$thsp_field_value['setting_args']))? $thsp_field_value['setting_args']['default'] : "",
